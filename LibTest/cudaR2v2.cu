...
/*__global__ void CUDA_Gen() {

}*/


__global__ void cudaR2v2_Add(double* c, double* a, double* b, int N) {
...
__global__ void cudaR2v2_Sub(double* c, double* a, double* b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a[i] - b[i];
	}
...
__global__ void cudaR2v2_Mult(double* c, double* a, double* b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a[i] * b[i];
	}
...
__global__ void cudaR2v2_Divide(double* c, double* a, double* b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		if (b[i] != 0.0) {
			c[i] = a[i] / b[i];
...
__global__ void cudaR2v2_Add_RHS(double* c, double* a, double b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a[i] + b;
	}
...
__global__ void cudaR2v2_Sub_RHS(double* c, double* a, double b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a[i] - b;
	}
...
__global__ void cudaR2v2_Mult_RHS(double* c, double* a, double b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a[i] * b;
	}
...
__global__ void cudaR2v2_Divide_RHS(double* c, double* a, double b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a[i] / b;
	}
...
__global__ void cudaR2v2_Add_LHS(double* c, double a, double* b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a + b[i];
	}
...
__global__ void cudaR2v2_Sub_LHS(double* c, double a, double* b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a - b[i];
	}
...
__global__ void cudaR2v2_Mult_LHS(double* c, double a, double* b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		c[i] = a * b[i];
	}
...
__global__ void cudaR2v2_Divide_LHS(double* c, double a, double* b, int N) {
	int i = blockDim.x*blockIdx.x + threadIdx.x;
	if (i < N) {
		if (b[i] != 0.0) {
			c[i] = a / b[i];
...
//Wurks
cudaR2v2::cudaR2v2(const cudaR2v2 & rhs) {
...
cudaR2v2::cudaR2v2(cudaR2v2 && inp) {
	/* r-value constructor move pointers
	DO NOT CONSTRUCT FROM AN ALREADY CONSTRUCTED
	FUNCTION! i.e. function already created, cudaR2v2 F;
...
//WARNING CHECK R-VALUES
cudaR2v2 & cudaR2v2::operator=(cudaR2v2 && rhs) {
	delete[] h_ptr;
	h_ptr = rhs.h_ptr;
	cudaFree(d_ptr);
	d_ptr = rhs.d_ptr;
...
cudaR2v2::cudaR2v2(int k, double X1, double X2, int M, double Y1, double Y2, int N) {
...
cudaR2v2 cudaR2v2::operator+(const cudaR2v2 & rhs) {
	...
	if (sameDim(*this, rhs)) {
		...
		//Execute GPU duals:
		cudaR2v2_Add << <grid, block >> >(temp.getD_ptr(), d_ptr, rhs.d_ptr, rhs.getSize());
...
cudaR2v2 cudaR2v2::operator-(const cudaR2v2 & rhs) {
	...
	if (sameDim(*this, rhs)) {
		...
		temp.setG(grid); temp.setB(block);
		//Execute GPU duals:
		cudaR2v2_Sub << <grid, block >> >(temp.getD_ptr(), d_ptr, rhs.d_ptr, rhs.getSize());
...
cudaR2v2 cudaR2v2::operator*(const cudaR2v2 & rhs) {
	...
	if (sameDim(*this, rhs)) {
		...
		temp.setG(grid); temp.setB(block);
		//Execute GPU duals:
		cudaR2v2_Mult << <grid, block >> >(temp.getD_ptr(), d_ptr, rhs.d_ptr, rhs.getSize());
...
cudaR2v2 cudaR2v2::operator/(const cudaR2v2 & rhs) {
	...
	if (sameDim(*this, rhs)) {
		...
		temp.setG(grid); temp.setB(block);
		//Execute GPU duals:
		cudaR2v2_Divide << <grid, block >> >(temp.getD_ptr(), d_ptr, rhs.d_ptr, rhs.getSize());
...
cudaR2v2 operator+(const cudaR2v2 & lhs, double rhs) {
	cudaR2v2 temp(lhs.m, lhs.n);
	...
	//Execute GPU duals:
	cudaR2v2_Add_RHS << <lhs.grid, lhs.block >> >(temp.getD_ptr(), lhs.d_ptr, rhs, lhs.getSize());
...
cudaR2v2 operator-(const cudaR2v2 & lhs, double rhs) {
	...
	temp.setG(lhs.grid); temp.setB(lhs.block);
	//Execute GPU duals:
	cudaR2v2_Sub_RHS << <lhs.grid, lhs.block >> >(temp.getD_ptr(), lhs.d_ptr, rhs, lhs.getSize());
...
cudaR2v2 operator*(const cudaR2v2 & lhs, double rhs) {
	...
	temp.setG(lhs.grid); temp.setB(lhs.block);
	//Execute GPU duals:
	cudaR2v2_Mult_RHS << <lhs.grid, lhs.block >> >(temp.getD_ptr(), lhs.d_ptr, rhs, lhs.getSize());
...
cudaR2v2 operator/(const cudaR2v2 & lhs, double rhs) {
	...
	if (rhs != 0.0) {
		...
		temp.setG(lhs.grid); temp.setB(lhs.block);
		//Execute GPU duals:
		cudaR2v2_Divide_RHS << <lhs.grid, lhs.block >> > (temp.getD_ptr(), lhs.d_ptr, rhs, lhs.getSize());
...
cudaR2v2 operator+(double lhs, const cudaR2v2 & rhs) {
	cudaR2v2 temp(rhs.m, rhs.n);
	...
	//Execute GPU duals:
	cudaR2v2_Add_LHS << <rhs.grid, rhs.block >> >(temp.getD_ptr(), lhs, rhs.d_ptr, rhs.getSize());
...
cudaR2v2 operator-(double lhs, const cudaR2v2 & rhs) {
	...
	temp.setG(rhs.grid); temp.setB(rhs.block);
	//Execute GPU duals:
	cudaR2v2_Sub_LHS << <rhs.grid, rhs.block >> >(temp.getD_ptr(), lhs, rhs.d_ptr, rhs.getSize());
...
cudaR2v2 operator*(double lhs, const cudaR2v2 & rhs) {
	...
	temp.setG(rhs.grid); temp.setB(rhs.block);
	//Execute GPU duals:
	cudaR2v2_Mult_LHS << <rhs.grid, rhs.block >> >(temp.getD_ptr(), lhs, rhs.d_ptr, rhs.getSize());
...
cudaR2v2 operator/(double lhs, const cudaR2v2 & rhs) {
	...
	temp.setG(rhs.grid); temp.setB(rhs.block);
	//Execute GPU duals:
	cudaR2v2_Divide_LHS << <rhs.grid, rhs.block >> >(temp.getD_ptr(), lhs, rhs.d_ptr, rhs.getSize());
...
cudaR2v2 d_dx(const cudaR2v2 & f, int Ver) {
	cudaR2v2 temp(f);
	temp.call2Host();
	int M = f.m, N = f.n;
	if (Ver == 1) {
...
cudaR2v2 d_dy(const cudaR2v2 & f, int Ver) {
	cudaR2v2 temp(f);
	temp.call2Host();
	int M = f.m, N = f.n;
	if (Ver == 1) {
